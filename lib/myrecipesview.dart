...
class _MyRecipesViewState extends State<MyRecipesView> {
  ...
  Future<void> _createRecipeAndOpen() async {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CurrentRecipeView(recipeId: id, title: name),
      ),
    );
  }
                ...
                child: _recipes.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          // choose a translucent card color that adapts to light/dark theme
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              // delete is now on long-press to avoid always showing a trash icon
                              onLongPress: () async {
                                final confirm = await showDialog<bool>(
                                  context: context,
                                  builder: (ctx) => AlertDialog(
                                    title: const Text('Potvrdi brisanje'),
                                    content: Text('Obrisati "${r['name']}"?'),
                                    actions: [
                                ...
                                if (confirm == true) _removeRecipeAt(i);
                              },
                            ),
                          );
                        },
